...
NBRTR.V          = vector("list",N)
for (n in 1:N){
  NBRTR.V[[n]]   = cbind(diag(n),matrix(0,n,N-n))
}
NBRTR.V[[4]]     = NBRTR.V[[6]]
NBRTR.V[[5]]     = NBRTR.V[[6]][-4,]
NBRTR.V[[6]]     = NBRTR.V[[6]][-c(4,5),]

B0.initial = matrix(0,N,N)
for (n in 1:N){
  unrestricted    = apply(NBRTR.V[[n]],2,sum)==1
...
t0                  = proc.time()
B0.posterior        = rgn(n=S.burnin, S=S.post, nu=nu.post, V=NBRTR.V, B0.initial=B0.initial)
t1                  = proc.time()
(t1-t0)/60
# sampling B0 from the posterior distribution using Gibbs
t0                  = proc.time()
B0.posterior        = rgn(n=S, S=S.post, nu=nu.post, V=NBRTR.V, B0.initial=B0.posterior[,,S.burnin])
t1                  = proc.time()
(t1-t0)/60
# normalisation
B0.hat              = diag(sign(diag(B0.posterior[,,S])))%*%B0.posterior[,,S]
NBRTR.B0.posterior    = normalize.Gibbs.output.parallel(B0.posterior,B0.hat=B0.hat)
t2                  = proc.time()
(t2-t1)/60

# sample B+ from the normal conditional posterior
t2                  = proc.time()
NBRTR.Bp.posterior    = rnorm.ngn(NBRTR.B0.posterior, B=B.post,Omega=Omega.post)
t3                  = proc.time()
(t3-t2)/60

save(NBRTR.B0.posterior,NBRTR.Bp.posterior, priors, posteriors, file="NBRTR-posterior.RData")
...
for (s in 1:S){
  B               = solve(NBRTR.B0.posterior[,,s])
  B.posterior[,,s]= B
  A.posterior[,,s]= B %*% NBRTR.Bp.posterior[,,s]
}
...
save(IRF.posterior,IRF.inf.posterior, FEVD.posterior, file="NBRTR-irf.RData")
...
# plot IRFs and FEVDs
############################################################
IRF.posterior.mps = IRF.posterior[,5,,]
...
pdf(file="NBRTR-irf-mps.pdf", height=9, width=12)
