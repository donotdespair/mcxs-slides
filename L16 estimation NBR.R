...
NBR.V          = vector("list",N)
for (n in 1:N){
  NBR.V[[n]]   = cbind(diag(n),matrix(0,n,N-n))
}
NBR.V[[4]]     = NBR.V[[5]]
NBR.V[[5]]     = NBR.V[[5]][-4,]
...
for (n in 1:N){
  unrestricted    = apply(NBR.V[[n]],2,sum)==1
...
t0                  = proc.time()
B0.posterior        = rgn(n=S.burnin, S=S.post, nu=nu.post, V=NBR.V, B0.initial=B0.initial)
t1                  = proc.time()
(t1-t0)/60
# sampling B0 from the posterior distribution using Gibbs
t0                  = proc.time()
B0.posterior        = rgn(n=S, S=S.post, nu=nu.post, V=NBR.V, B0.initial=B0.posterior[,,S.burnin])
t1                  = proc.time()
(t1-t0)/60
# normalisation
B0.hat              = diag(sign(diag(B0.posterior[,,S])))%*%B0.posterior[,,S]
NBR.B0.posterior    = normalize.Gibbs.output.parallel(B0.posterior,B0.hat=B0.hat)
t2                  = proc.time()
(t2-t1)/60

# sample B+ from the normal conditional posterior
t2                  = proc.time()
NBR.Bp.posterior    = rnorm.ngn(NBR.B0.posterior, B=B.post,Omega=Omega.post)
t3                  = proc.time()
(t3-t2)/60

save(NBR.B0.posterior,NBR.Bp.posterior, priors, posteriors, file="NBR-posterior.RData")
...
for (s in 1:S){
  B               = solve(NBR.B0.posterior[,,s])
  B.posterior[,,s]= B
  A.posterior[,,s]= B %*% NBR.Bp.posterior[,,s]
}
...
save(IRF.posterior,IRF.inf.posterior, FEVD.posterior, file="NBR-irf.RData")
...
pdf(file="NBR-irf-mps.pdf", height=9, width=12)
