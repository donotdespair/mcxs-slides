...
pdf(file="data-us.pdf",height=5, width=8.5)
plot(y[], main="", xlab="", nc=2, lwd=2, col=mcxs1)
dev.off()
...
t0                  = proc.time()
B0.posterior        = rgn(n=S.burnin, S=S.post, nu=nu.post, V=FF.V, B0.initial=B0.initial)
t1                  = proc.time()
(t1-t0)/60
# sampling B0 from the posterior distribution using Gibbs
t0                  = proc.time()
B0.posterior        = rgn(n=S, S=S.post, nu=nu.post, V=FF.V, B0.initial=B0.posterior[,,S.burnin])
t1                  = proc.time()
(t1-t0)/60
# normalisation
