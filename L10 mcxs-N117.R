...
# using 117 macro variables
# For info about the variables, their transformations and data sources see:
# Panagiotelis, Athanasopoulos, Hyndman, Jiang, Vahid (2019) Macroeconomic forecasting for Australia using a large number of predictors, Interantional Journal of Forecasting
############################################################
rm(list=ls())
library(mvtnorm)
# library(plot3D)
...
inflation.range = range(Y[,2])

pdf(file="data.pdf", width=15,height=6)
par(mfrow=c(1,2), mar=rep(2.2,4),cex.axis=1.5)
plot(1:length(Y[,1]),Y[,1], type="l", ylim=rgdp.range, axes=FALSE, xlab="", ylab="", lwd=2, col=mcxs1, main=expression(rgdp[t]))
axis(1,c(1,4,16,56,96,120),c("","1987","1990","2000","2010","2016"), col=mcxs1)
axis(2,c(rgdp.range[1],0,rgdp.range[2]),c("","",""), col=mcxs1)
abline(h=0, col=mcxs1)
plot(1:length(Y[,2]),Y[,2], type="l", ylim=inflation.range, axes=FALSE, xlab="", ylab="", lwd=2, col=mcxs2, main=expression(cpi[t]))
axis(1,c(1,4,16,56,96,120),c("","1987","1990","2000","2010","2016"), col=mcxs1)
axis(2,c(inflation.range[1],0,inflation.range[2]),c("","",""), col=mcxs1)
abline(h=0, col=mcxs1)
dev.off()
...
t1  = proc.time()
(t1-t0)/60
# save(Y.h,Y.h.m,file="forecasts-N117-p1.RData")
# save(Y.h,Y.h.m,file="forecasts-N117-p2.RData")
save(Y.h,Y.h.m,file="forecasts-N117-p4.RData")


# plots
############################################################
# plot rgdp growth forecasts
load("forecasts-N117-p1.RData")
Yh1        = Y.h
Yhm1       = Y.h.m
load("forecasts-N117-p2.RData")
Yh2         = Y.h
Yhm2        = Y.h.m
load("forecasts-N117-p4.RData")
Yh4         = Y.h
Yhm4        = Y.h.m

Y.recent        = ts(rbind(Y[92:117,1:2],matrix(NA,h,2)),start=c(2009,1),frequency=4)
interval.fs1    = apply(Yh1,1:2,hdi,credMass=0.68)
interval.fs2    = apply(Yh2,1:2,hdi,credMass=0.68)
interval.fs4    = apply(Yh4,1:2,hdi,credMass=0.68)
lims.gdp.s      = range(Y.recent[1:26,1],Yhm1[,1],Yhm2[,1],Yhm4[,1])
lims.pi.s       = range(Y.recent[1:26,2],Yhm1[,2],Yhm2[,2],Yhm4[,2])

# median.forecasts.s1  = apply(Yh02,1:2,quantile,probs=0.5)
# median.forecasts.s1   = apply(Yh1,1:2,quantile,probs=0.5)

pdf(file=paste("forecasts-N117.pdf",sep=""), width=15,height=6)
...
polygon(c(26:30,30:26), c(Y.recent[26,1],interval.fs2[1,,1],interval.fs2[2,h:1,1],Y.recent[26,1]), col=mcxs1.shade1,border=mcxs1.shade1)
abline(h=0, col=mcxs1)
lines(26:30, c(Y.recent[26,1],Yhm1[,1]),lwd=2,col=mcxs3)
lines(26:30, c(Y.recent[26,1],Yhm2[,1]),lwd=2,col=mcxs2)
lines(26:30, c(Y.recent[26,1],Yhm4[,1]),lwd=2,col=mcxs1)

plot(1:nrow(Y.recent),Y.recent[,2], type="l", ylim=lims.pi.s, axes=FALSE, xlab="", ylab="", main=expression(cpi[t]), lwd=2, col=mcxs2)
axis(1,c(seq(from=1, to=30, by=4),30),c("","2010","","2012","","2014","","2016",""), col=mcxs1)
axis(2,c(lims.pi.s[1],0,lims.pi.s[2]),round(c(lims.pi.s[1],0,lims.pi.s[2]),2), col=mcxs1)
polygon(c(26:30,30:26), c(Y.recent[26,2],interval.fs1[1,,2],interval.fs1[2,h:1,2],Y.recent[26,2]), col=mcxs1.shade1,border=mcxs1.shade1)
abline(h=0, col=mcxs1)
lines(26:30, c(Y.recent[26,2],Yhm1[,2]),lwd=2,col=mcxs3)
lines(26:30, c(Y.recent[26,2],Yhm2[,2]),lwd=2,col=mcxs2)
lines(26:30, c(Y.recent[26,2],Yhm4[,2]),lwd=2,col=mcxs1)
dev.off()
